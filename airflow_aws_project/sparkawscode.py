...
def spark_code(bucket_name,file_key):

    spark = SparkSession \
    .builder \
    .appName("maarketdata") \
    .config("spark.app.name", "S3app") \
    .config("spark.jars.packages", "org.apache.hadoop:hadoop-aws:2.7.7,org.apache.hadoop:hadoop-common:2.7.7") \
    .config("spark.hadoop.fs.s3a.access.key", "AKIA3LA4MK5MJQTYUDEO") \
    .config("spark.hadoop.fs.s3a.secret.key", "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") \
    ...
    s3_file_path = "s3a://{}/{}".format(bucket_name, file_key)
    data = spark.read.csv(s3_file_path,schema=schema,sep=",",header=True)

    # Output S3 bucket details
    s3_bucket = 'timberbucket'
    s3_output_path = 's3a://{}/outputfiles/'.format(s3_bucket)
    df_output = "{}/df_output.csv".format(s3_output_path)
    # data.printSchema()
    # data.show()
